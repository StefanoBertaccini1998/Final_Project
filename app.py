...
def _enet_classify(image_float: np.ndarray, category: str) -> tuple[str, float, np.ndarray | None]:
    """Return (verdict, prob, gradcam_overlay) for EfficientNet."""
...
def classify(image: np.ndarray | None, category: str) -> tuple:
    """Run both models on the input image and return all outputs."""
    ...
    # EfficientNet
    enet_verdict, enet_prob, gradcam_overlay = _enet_classify(image_float, category)
    enet_prob_str = f"{enet_prob:.1%}  (threshold {ENET_THRESHOLD:.0%})"

    # Grad-CAM image (fallback to original if unavailable)
    if gradcam_overlay is None:
        gradcam_overlay = (image_float * 255).astype(np.uint8)
    ...
    return roi_img, hog_verdict, hog_prob_str, gradcam_overlay, enet_verdict, enet_prob_str, warning_html
...
with gr.Blocks(title="Smart Factory Vision Monitor", theme=gr.themes.Default()) as demo:
    ...
    gr.Markdown(
        ...
        "*EfficientNet threshold: 0.3 (optimised for recall). "
